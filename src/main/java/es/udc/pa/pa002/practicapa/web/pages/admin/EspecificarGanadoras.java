...
public class EspecificarGanadoras {
	...
	void onActivate(Long idTipoApuesta){
		...
		if(tipoApuesta!=null)
			opcionApuestaSelectModel = selectModelFactory.create(this.getOpcionesApuesta(),"respuesta");
	...
	void onValidateFromEspecificarGanadorasForm () {
		...
		try {
			...
			especificarGanadorasForm.recordError(messages.format("error-eventoNotStarted", tipoApuesta.getEvento().getNombre()));
		} catch (OpcionApuestaAlreadySolvedException e) {
			especificarGanadorasForm.recordError(messages.format("error-alreadySolved", tipoApuesta.getPregunta()));
		} catch (SimpleWinnerException e) {
			especificarGanadorasForm.recordError(messages.format("error-simpleWinner", tipoApuesta.getPregunta()));
		} catch (InvalidOptionException e) {
			especificarGanadorasForm.recordError(messages.format("error-invalidOption", tipoApuesta.getPregunta()));
		}
