...
public class AnadirEvento {
    ...
    boolean validateFecha(String fecha){
        return Integer.valueOf(fecha.substring(0, 2))>23 || Integer.valueOf(fecha.substring(3))>60;
    }
    ...
    void OnValidateFromAddEventosForm(){
        ...
        if(validateFecha(hora)){
            addEventosForm.recordError(horaField,messages.get("error-invalidTime"));
            return;
        }
        ...
        try {
            ...
            addEventosForm.recordError(fechaDateField,messages
                    .get("error-invalidDate"));
            
        } catch (InstanceAlreadyCreatedException e) {
            addEventosForm.recordError(messages
                    .get("error-eventocreated"));
