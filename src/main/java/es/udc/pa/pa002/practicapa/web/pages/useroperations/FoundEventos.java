...
public class FoundEventos {
	...
	void onActivate(String keywords,Long idCategoria, int startIndex) {
		...
		if(userSession!=null){
			eventoGridDataSource=new EventoGridDataSource(userService,keywords,idCategoria,userSession.isAdmin());
			eventoBlock = userService.findEventos(keywords,idCategoria,userSession.isAdmin(),startIndex, EVENTOS_PER_PAGE);
		}
		else{
			eventoGridDataSource=new EventoGridDataSource(userService,keywords,idCategoria,false);
			eventoBlock = userService.findEventos(keywords,idCategoria,false,startIndex, EVENTOS_PER_PAGE);
		}
	}
